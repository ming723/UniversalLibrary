...
public  class FragmentHttpUtils {
    private RxAppCompatDialogFragment activity;
    public FragmentHttpUtils(RxAppCompatDialogFragment activity){
      this.activity=activity;
    }
    private boolean isReadCache=true,isShow=true;
    public FragmentHttpUtils(RxAppCompatDialogFragment activity, boolean isReadCache, boolean isShow){
        this.activity=activity;
    ...
    private void doMethod(String url, Map<String,String> map,int type){
        FragmentUniversalApi getEntity = new FragmentUniversalApi(simpleOnNextListener,activity, type,url,map);
        getEntity.readCacle(isReadCache);
        getEntity.isProgress(isShow);
        FragmentHttpManager manager = FragmentHttpManager.getInstance();
    ...
    private  <D extends IModel> void doJavaBeanMethod(String url, Map<String,String> map,int type){
        FragmentUniversalApi getEntity = new FragmentUniversalApi(new HttpOnNextListener<ResponseBody>() {
            @Override
            public void onNext(ResponseBody responseBody) {
                try {
